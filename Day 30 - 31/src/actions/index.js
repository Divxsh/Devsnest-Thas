const GetProduct = () => {
	return (dispatch) => {
		fetch(
			"https://v1.nocodeapi.com/coderbhai/airtable/bIORwIpXcVhypdcq?tableName=products",
		)
			.then((res) => res.json())
			.then((data) => {
				dispatch({
...
const GetProductDetails = (id) => {
	return (dispatch) => {
		fetch(
			`https://v1.nocodeapi.com/coderbhai/airtable/bIORwIpXcVhypdcq?tableName=products&id=${id}`,
		)
			.then((res) => res.json())
			.then((datas) => {
